            ...
            <div className='relative'>
                <div className='flex  justify-between custom-shadow py-3 px-3 place-items-center bg-white'>
                    ...
                    <div className=' justify-evenly gap-3 text-[1rem] place-items-center hidden sm:flex font-semibold'>
                        <Link href='/' className={`hover:after:content-[' '] hover:after:absolute hover:after:w-2 hover:after:h-2 hover:after:bg-orange hover:after:rounded-full hover:after:bottom-[-0.5rem] hover:after:ml-[-1.8rem] after:mt-[2rem] relative hover:text-orange transition-colors ease-out duration-75 ${currentTab === 'home' ? "after:content-[' '] after:absolute after:w-2 after:h-2 after:bg-orange after:rounded-full after:bottom-[-0.5rem] after:ml-[-1.8rem] after:mt-[2rem] relative text-orange" : ''}`}>HOME</Link>

                        <Link href='/pricing' className={`hover:after:content-[' '] hover:after:absolute hover:after:w-2 hover:after:h-2 hover:after:bg-orange hover:after:rounded-full hover:after:bottom-[-0.5rem] hover:after:ml-[-2.3rem] after:mt-[2rem] relative hover:text-orange transition-colors ease-out duration-75 ${currentTab === 'pricing' ? "after:content-[' '] after:absolute after:w-2 after:h-2 after:bg-orange after:rounded-full after:bottom-[-0.5rem] after:ml-[-2.3rem] after:mt-[2rem] relative text-orange" : ''}`}>PRICING</Link>
                        
                        <Link href='/contact-us' className={`hover:after:content-[' '] hover:after:absolute hover:after:w-2 hover:after:h-2 hover:after:bg-orange hover:after:rounded-full hover:after:bottom-[-0.5rem] hover:after:ml-[-3.7rem] after:mt-[2rem] relative hover:text-orange transition-colors ease-out duration-75 ${currentTab === 'contact-us' ? "after:content-[' '] after:absolute after:w-2 after:h-2 after:bg-orange after:rounded-full after:bottom-[-0.5rem] after:ml-[-3.7rem] after:mt-[2rem] relative text-orange" : ''}`}>CONTACT US</Link>

                        <Link href='/sign-up' className={`hover:after:content-[' '] hover:after:absolute hover:after:w-2 hover:after:h-2 hover:after:bg-green hover:after:rounded-full hover:after:bottom-[-0.5rem] hover:after:ml-[-2.3rem] after:mt-[2rem] relative hover:text-green transition-colors ease-out duration-75 text-green ${currentTab === 'sign-up' ? "after:content-[' '] after:absolute after:w-2 after:h-2 after:bg-green after:rounded-full after:bottom-[-0.5rem] after:ml-[-2.3rem] after:mt-[-2rem] relative text-green" : ''}`}>SIGN UP</Link>
                    </div>
                ...
                <div className={`absolute w-[15rem] bg-white flex justify-center place-content-evenly flex-col gap-3 top-[5.3rem] ml-1 right-0 p-3 custom-shadow transition-opacity ease-in-out duration-300 ${menuToggle ? 'opacity-100' : 'opacity-0 pointer-events-none'} sm:hidden`}>
                    ...
                    <Link href='/login' className='bg-orange flex rounded-sm justify-center place-items-center gap-1 text-[1rem] text-white px-6 py-2 font-semibold'>
                        <p>LOG IN</p>
                        <Image src={Images.signIn} alt='signin' height={15} priority={true} />
                    </Link>
                </div>
